    <template>
      <div class="community-programs">
        ...
        <!-- Modal chi tiết -->
        <div v-if="selectedEvent && !showForm" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div v-if="!isAdmin()">
              <button v-if="joinedStatus === false" :disabled="loadingJoin" @click="handleJoinProgram(selectedEvent.id)">
    ...
    watchEffect(() => {
      console.log('watchEffect - localStorage role:', localStorage.getItem('role'));
      console.log('watchEffect - isAdmin:', isAdmin());
    });
    ...
    async function checkParticipation(programId) {
      ...
      try {
        const token = localStorage.getItem('token');
        // Gọi API lấy danh sách chương trình user đã tham gia (hoặc BE có API check nhanh hơn thì dùng)
        // Ở đây giả định BE trả về danh sách, FE check có programId không
    ...
    async function fetchParticipants(programId) {
      ...
      try {
        const token = localStorage.getItem('token');
        const res = await getParticipantsByProgram(programId, token);
    ...
    function openModal(event) {
      ...
      if (isAdmin()) fetchParticipants(event.id);
    }
    function closeModal() {
    ...
    const addProgram = async () => {
      try {
        const token = localStorage.getItem('token');
        // Đảm bảo gửi đúng định dạng ISO string
        const data = {
          ...form.value,
