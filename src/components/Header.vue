...
<style scoped>
.main-header {
  background-color: rgba(255, 255, 255, 0.9); /* Subtle transparency */
  box-shadow: var(--shadow-1); /* Light shadow */
  padding: var(--spacing-md) 0; /* More vertical padding */
  position: sticky;
  top: 0;
  z-index: 1000;
...
.container {
  ...
  padding: 0 var(--spacing-lg); /* More horizontal padding */
}
.logo a {
...
.navbar {
  display: flex;
  gap: var(--spacing-xs); /* Further reduced gap */
  align-items: center;
...
.navbar a {
  ...
  padding: var(--spacing-xs) var(--spacing-sm); /* Reduced horizontal padding */
...
.user-info {
  ...
  margin-left: var(--spacing-md); /* Adjusted space from other nav items */
  white-space: nowrap; /* Prevent text from wrapping */
}
...
@media (max-width: 768px) {
  ...
  .navbar {
    ...
    padding-bottom: var(--spacing-xs); /* Add some padding for scrollbar */
  }
  .navbar a,
  .user-info {
  ...
  .user-info {
    ...
    padding: var(--spacing-xs) 0; /* Add some padding */
    border-top: 1px solid var(--background-color); /* Separator */
  }
}
</style>
